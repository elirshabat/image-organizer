...
from utils import (file_hash, list_media_files, create_logger, get_logger, list_new_media_files, is_path_in_list,
                   order_files)
...
def _add_single_file_to_index(filepath, repo_dir, hash_dict, meta):
...
def _add_files_to_index(hash_dict, meta, media_files, repo_dir, show_progress=False, progress_msg=None):
    logger = get_logger(LOGGER_NAME)
    ...
    for filepath in files_iterator:
        try:
            _add_single_file_to_index(filepath, repo_dir, hash_dict, meta)
...
def create_index(repo_dir):
    ...
    hash_dict = dict()
    _add_files_to_index(hash_dict, meta, media_files, repo_dir, show_progress=True, progress_msg="Creating index")
...
def _update_error_files(repo_dir, hash_dict, meta):
    ...
    for i in range(len(error_files) - 1, -1, -1):
        ...
        if not os.path.exists(full_path):
            ...
            try:
                _add_single_file_to_index(full_path, repo_dir, hash_dict, meta)
...
def update_index(repo_dir, show_progress=False, force=False):
    ...
    _add_files_to_index(hash_dict, meta, new_media_files, repo_dir,
